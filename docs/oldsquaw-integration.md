...
- 需要宿主工作区注入的项目先做适配器入口，明确后续要打包的资源边界。
...
| BetterNemo | 不是普通网页。`workspace.html` 依赖 assets、theme、extensions、workspace-scripts 和 `extension-loader.js` 注入流程。 | 当前提供 `public/tools/oldsquaw/better-nemo/index.html` 适配器说明，下一步应打包 workspace 资源后再改为可运行编辑器。 |
...
- 为 BetterNemo 建立 workspace 级打包：只引入运行必需的 `workspace.html`、assets、theme、extensions、workspace-scripts 与加载器配置。
